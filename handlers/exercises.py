...
class ExerciseState(StatesGroup):
    waiting_for_data = State()  # Ждем: вес повторения подходы
...
async def process_data(message: types.Message, state: FSMContext):
    ...
    try:
        if has_weight:
            if len(raw) != 3:
                await message.answer("❌ Нужно 3 числа: вес повторения подходы. Пример: 20 10 4")
                return
            ...
            if len(raw) != 2:
                await message.answer("❌ Нужно 2 числа: повторения подходы. Пример: 10 4")
                return
            weight = 0
            reps = int(raw[0])
    ...
    completed = get_completed_exercises(user_id, day)
    total = len([ex for ex in PROGRAMS[program][day] if ex['sets'] > 0])
    ...
    if len(completed) >= total:
        ...
        await message.answer(
            f"{summary}\n\nОсталось: {total - len(completed)} упражнений",
